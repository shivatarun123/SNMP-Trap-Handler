...
my @password = split('"', $lines[4]);
 ...
 or die "Connection Error: $DBI::errstr\n";
	...
	$sth = $dbh->prepare("CREATE TABLE IF NOT EXISTS `traps`
...
$time=time();
	...
	$trapVerify = $dbh->prepare("select * from `traps` where `IP`='$ip'");
	$trapVerify->execute;
	...
	$trapPrev = $dbh->prepare("select `STATUS`,`TIME`, `STATUS1` from `traps` where `IP`='$ip'");
	$trapPrev->execute;
	...
	$trapUpdate = $dbh->prepare("UPDATE `traps` SET  		`STATUS`='$status',`TIME`='$time',`PREVIOUS_STATUS`='$previousStatus',`PREVIOUS_TIME`='$previousTime',`STATUS1`='$status1',
`PREVIOUS_STATUS1`='$previousStatus1'  WHERE `IP` = '$ip'");
	...
	$trapNew = $dbh->prepare("insert into `traps` (IP,STATUS,TIME,STATUS1,PREVIOUS_STATUS1) VALUES ('$ip','$status','$time', '$status1','-')");	
	$trapNew->execute;
...
$trapDevice = $dbh->prepare("select * from `trap device`");
	$trapDevice->execute;
...
#Preparing FAIL status
	 $trapFail = $dbh->prepare("select * from `traps` WHERE `STATUS` = 'FAIL' AND `IP` = '$ipFAIL'");
	 $trapFail->execute;
...
#preparing DANGER status
	 ...
	 $trapDanger = $dbh->prepare("select * from `traps` WHERE `STATUS` = 'DANGER'");
	 $trapDanger->execute;
